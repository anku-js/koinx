export const trendingTokenResponse = [
  {
    id: 1,
    symbol: "BNB",
    icon: "/images/icon1.png",
    percentChange: 0.52,
    currentPrice: "$319.34",
    graph: "/images/graph1.png",
  },
  {
    id: 2,
    symbol: "SOL",
    icon: "/images/icon2.png",
    percentChange: 2.37,
    currentPrice: "$319.34",
    graph: "/images/graph2.png",
  },
  {
    id: 3,
    symbol: "XRP",
    icon: "/images/icon3.png",
    percentChange: 1.23,
    currentPrice: "$319.34",
    graph: "/images/graph3.png",
  },
  {
    id: 4,
    symbol: "ADA",
    icon: "/images/icon4.png",
    percentChange: 2.43,
    currentPrice: "$319.34",
    graph: "/images/graph4.png",
  },
  {
    id: 5,
    symbol: "AVAX",
    icon: "/images/icon5.png",
    percentChange: 0.12,
    currentPrice: "$319.34",
    graph: "/images/graph5.png",
  },
  {
    id: 6,
    symbol: "BTC",
    icon: "/images/icon6.png",
    percentChange: -1.52,
    currentPrice: "$319.34",
    graph: "/images/graph6.png",
  },
  {
    id: 7,
    symbol: "ETH",
    icon: "/images/icon7.png",
    percentChange: 0.45,
    currentPrice: "$319.34",
    graph: "/images/graph7.png",
  },
  {
    id: 8,
    symbol: "stETH",
    icon: "/images/icon8.png",
    percentChange: -1.67,
    currentPrice: "$319.34",
    graph: "/images/graph8.png",
  },
];

export const currencies = {
  id: 1,
  heading: "About Bitcoin",
  subheading: "What is Bitcoin?",
  description:
    "Bitcoin’s price today is US$16,951.82, with a 24-hour trading volume of $19.14 B. BTC is +0.36% in the last 24 hours. It is currently -7.70% from its 7-day all-time high of $18,366.66, and 3.40% from its 7-day all-time low of $16,394.75. BTC has a circulating supply of 19.24 M BTC and a max supply of 21 M BTC.",
  heading2: "Lorem ipsum dolor sit amet",
  description2_pointer1:
    "Lorem ipsum dolor sit amet consectetur. Aliquam placerat sit lobortis tristique pharetra. Diam id et lectus urna et tellus aliquam dictum at. Viverra diam suspendisse enim facilisi diam ut sed. Quam scelerisque fermentum sapien morbi sodales odio sed rhoncus. Ultricies urna volutpat pendisse enim facilisi diam ut sed. Quam scelerisque fermentum sapien morbi sodales odio sed rhoncus.",
  description2_pointer2:
    "Diam praesent massa dapibus magna aliquam a dictumst volutpat. vitae pellentesque auctor amet. Nunc sagittis libero adipiscing cursus felis pellentesque interdum. Odio cursus phasellus velit in senectus enim dui. Turpis tristique placerat interdum sed volutpat. Id imperdiet magna eget eros donec cursus nunc. Mauris faucibus diam mi nunc praesent massa turpis a. Integer dignissim augue viverra nulla et quis lobortis phasellus. Integer pellentesque enim convallis ultricies at.",
  description2_pointer3:
    "Fermentum hendrerit imperdiet nulla viverra faucibus. Sit aliqua massa vel convallis duis ac. Mi adipiscing semper scelerisque porttitor pulvinar nunc risus. Fermentum potenti iaculis lacinia congue ipsum fames amet dui. Purus ultrices tincidunt volutpat in eget. Ullamcorper dui",
  heading3: "Already Holding Bitcoin?",
  card_details: {
    profit: {
      heading: "Calculate your Profits",
      img: "/images/img1.jpg",
    },
    liability: {
      heading: "Calculate your liability",
      img: "/images/img2.jpg",
    },
  },
  summary:
    "Fermentum hendrerit imperdiet nulla viverra faucibus. Sit aliquam massa vel convallis duis ac. Mi adipiscing semper scelerisque porttitor pulvinar nunc risus. Fermentum potenti iaculis lacinia congue ipsum fames amet dui. Purus ultrices tincidunt volutpat in eget. Ullamcorper dui",
};

export const tokenomicsDetails = {
  heading: "Tokenomics",
  heading2: "Initial Distribution",
  img: "/images/img3.jpg",
  investors: "Crowdsale investors: 80%",
  foundation: "Foundation: 20%",
  description:
    "Lorem ipsum dolor sit amet consectetur. Cras aliquet tristique ornare vestibulum nunc dignissim vel consequat. Leo etiam nascetur bibendum amet enim sit eget leo amet. At metus orci augue fusce eleifend lectus eu fusce adipiscing. Volutpat ultrices nibh sodales massa habitasse urna felis augue. Gravida aliquam fermentum augue eu. Imperdiet bibendum amet aliquam donec. Eget justo dui metus odio rutrum. Vel ipsum eget in at curabitur sem posuere facilisis vitae. Sed lorem sit mauris id eget arcu ut. Vulputate ipsum aliquet odio nisi eu ac risus.",
};

export const teamMemberInfo = [
  {
    id: 1,
    name: "John Smith",
    designation: "Designation here",
    image: "/images/Team1.jpg",
    description:
      "Lorem ipsum dolor sit amet consectetur. In justo rutrum sit sit fermentum ut libero hendrerit id. Tellus sit ornare netus sagittis in nunc convallis mattis maecenas. Tempus arcu leo sociis laoreet nec neque sed pellentesque viverra. Consectetur proin amet ut id facilisi quis consectetur. Tellus gravida ultricies feugiat sed eu egestas dolor est ipsum. Malesuada etiam mi gravida praesent interdu",
  },
  {
    id: 2,
    name: "Elina Williams",
    designation: "Designation here",
    image: "/images/Team2.jpg",
    description:
      "Lorem ipsum dolor sit amet consectetur. In justo rutrum sit sit fermentum ut libero hendrerit id. Tellus sit ornare netus sagittis in nunc convallis mattis maecenas. Tempus arcu leo sociis laoreet nec neque sed pellentesque viverra. Consectetur proin amet ut id facilisi quis consectetur. Tellus gravida ultricies feugiat sed eu egestas dolor est ipsum. Malesuada etiam mi gravida praesent interdu",
  },
  {
    id: 3,
    name: "John Smith",
    designation: "Designation here",
    image: "/images/Team3.jpg",
    description:
      "Lorem ipsum dolor sit amet consectetur. In justo rutrum sit sit fermentum ut libero hendrerit id. Tellus sit ornare netus sagittis in nunc convallis mattis maecenas. Tempus arcu leo sociis laoreet nec neque sed pellentesque viverra. Consectetur proin amet ut id facilisi quis consectetur. Tellus gravida ultricies feugiat sed eu egestas dolor est ipsum. Malesuada etiam mi gravida praesent interdu",
  },
];

export const keyEvent = [
  {
    id: 1,
    image: "/images/keyEvent1.png",
    title:
      "Lorem ipsum dolor sit amet consectetur. Dui vel quis dignissim mattis enim tincidunt.",
    description:
      "Lorem ipsum dolor sit amet consectetur. Ac phasellus risus est faucibus metus quis. Amet sapien quam viverra adipiscing condimentum. Ac consectetur et pretium in a bibendum in. Sed vitae sit nisi viverra natoque lacinia libero enim.",
  },
  {
    id: 1,
    image: "/images/keyEvent2.png",
    title:
      "Lorem ipsum dolor sit amet consectetur. Dui vel quis dignissim mattis enim tincidunt.",
    description:
      "Lorem ipsum dolor sit amet consectetur. Ac phasellus risus est faucibus metus quis. Amet sapien quam viverra adipiscing condimentum. Ac consectetur et pretium in a bibendum in. Sed vitae sit nisi viverra in a adipisinc metus quis del",
  },
];
